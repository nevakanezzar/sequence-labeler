...
def run_experiment(config_path):
    ...
    if config["path_train"] is not None and len(config["path_train"]) > 0:
        ...
        if len(split_points)>1:
            try:
                step_size = int(split_points[0] * config["multiple"])
                while len(sentences_train) < split_points[0]+step_size:
                    sentences_train+=sentences_train[split_points[0]:split_points[1]] #adding copies of set 2
            except:
                step_size = (split_points[0])  #defaulting to multiple = 1
    ...
    if config["path_train"] is not None and len(config["path_train"]) > 0:
        ...
        try:
            for epoch in xrange(config["epochs"]):
                ...
                random.shuffle(sentences_train[split_points[0]+1:])
                for mini_batch_number,i in enumerate(range(split_points[0],len(sentences_train),step_size)):
                    print("Epoch:"+str(epoch)+"\tmini-batch:"+str(mini_batch_number))
                    total_mini_batches+=1
                    sentences_train_mini_batch = sentences_train[:split_points[0]]+sentences_train[i:i+step_size]

                    results_train = process_sentences(sequencelabeler, sentences_train_mini_batch, testing=False, learningrate=learningrate, name="train", main_label_id=label2id[str(config["main_label"])], word2id=word2id, char2id=char2id, label2id=label2id, singletons=singletons, config=config, verbose=True)
                    ...
                    if ((epoch == 0 and mini_batch_number == 0) or (main_measure_type == "high" and results_dev[main_measure] > best_selector_value) 
                        ...
                        best_epoch = epoch
                        best_mb = mini_batch_number
                        best_mini_batch_cumulative = total_mini_batches
                        best_selector_value = results_dev[main_measure]
                        print("Saving...")
                        sequencelabeler.save(temp_model_path)
                    
                    if config["stop_if_no_improvement_for_epochs"] > 0 and (total_mini_batches - best_mini_batch_cumulative) >= config["stop_if_no_improvement_for_epochs"]:
                        print("No improvement within epoch for some time, end of training...")
                        break


                print("\nEND OF EPOCH\n")
                print("best_epoch and minibatch: " + str(best_epoch)+"-"+str(best_mb))
            ...
            print("Caught exception, exiting training, attempting to run tests...")
            error_flag = True


        # loading the best model so far
        print("Loading best model and running tests...")
        if config["epochs"] > 0:
