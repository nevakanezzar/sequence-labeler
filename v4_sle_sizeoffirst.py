...
def run_experiment(config_path):
    ...
    if config["path_train"] is not None and len(config["path_train"]) > 0:
        ...
        sentences_train, split_points_old = read_input_files(config["path_train"], splits=True)
        split_points = [i for i in range(0,len(sentences_train),split_points_old[0])]+[len(sentences_train)-1]
        random.shuffle(sentences_train[split_points[1]:])

        if DEBUG:
            print("split_points:",split_points)
            print("split_points_old:",split_points_old)
            print("len(sentences_train):",len(sentences_train))
    ...
    if config["path_train"] is not None and len(config["path_train"]) > 0:
        ...
        try:
            for epoch in xrange(config["epochs"]):
                ...
                for mini_batch_number,split_start in enumerate(split_points[:-1]):
                    split_end = split_points[mini_batch_number+1]
                    if DEBUG:
                        print(mini_batch_number,split_start,split_end,split_points)

                    random.shuffle(sentences_train[split_start:split_end])
                    print("Epoch:"+str(epoch)+"\tmini-batch:"+str(mini_batch_number))
                    total_mini_batches+=1
                    sentences_train_mini_batch = sentences_train[split_start:split_end]
