...
def read_input_files(file_paths,splits=False):
    """
    ...
    If splits=True, also returns a vector of 'split-points' by signifying the sentence_id of the last sentence of each file
    """
    split_points = []
    counter = -1
    sentences = []
    ...
    if splits == True:
        try:
            assert len(sentences) == (split_points[-1]+1)
        except:
            print("last split-point is not equal to total number of sentences, aborting!")
            print(split_points[-1]+1, len(sentences))
            raise
...
def create_batches_of_sentence_ids(sentences, max_batch_size, cutoff=0, percentage=0):
    """
    ...
    Creates batches by sampling percentage sentences in each batch from the first cutoff sentences. cutoff must be <= 50% of dataset.
    ...
    try:
        assert cutoff <= len(sentences) * 0.5
    except:
        print("Assertion failed! Cutoff is larger than 50%")
        print("Cutoff:",cutoff,"\tTotal sentences:",len(sentences))
        raise
    try:
        assert percentage >= 0
        assert percentage <= 1
    except:
        print("Assertion failed! Percentage",percentage," is out of bounds!")
        raise
    cutoff_batchsize = int(batch_size*percentage)
    larger_batchsize = batch_size - cutoff_batchsize


    sentence_ids_by_length_cutoff = collections.OrderedDict()
    for i in range(cutoff):
        length = len(sentences[i][0])
        if length not in sentence_ids_by_length_cutoff:
            sentence_ids_by_length_cutoff[length] = []
        sentence_ids_by_length_cutoff[length].append(i)

    sentence_ids_by_length = collections.OrderedDict()
    for i in range(cutoff,len(sentences),1):
        length = len(sentences[i][0])
    ...
    for sentence_length in sentence_ids_by_length:
        for i in range(0, len(sentence_ids_by_length[sentence_length]), larger_batchsize):
            try:
                sent_ids_from_cutoff = numpy.random.choice(sentence_ids_by_length_cutoff[sentence_length], min(cutoff_batchsize, len(sentence_ids_by_length_cutoff[sentence_length])), replace=False)
            except KeyError:  #in case sentence_length does not exist for cutoff db
                sent_ids_from_cutoff = []

            sent_ids = []
            sent_ids.extend(sentence_ids_by_length[sentence_length][i:i + larger_batchsize])
            sent_ids.extend(sent_ids_from_cutoff)
            if DEBUG:
                print(sent_ids, len(sent_ids), cutoff, sentence_length, sent_ids_from_cutoff, sentence_length in sentence_ids_by_length_cutoff)

            batches_of_sentence_ids.append(sent_ids)
    
    sent_lengths_cutoff_not_larger = set(sentence_ids_by_length_cutoff.keys()) - set(sentence_ids_by_length.keys())
    for sentence_length in sent_lengths_cutoff_not_larger:
        for i in range(0, len(sentence_ids_by_length_cutoff[sentence_length]), batch_size):
            sent_ids = []
            sent_ids.extend(sentence_ids_by_length_cutoff[sentence_length][i:i + batch_size])
            if DEBUG:
                print(sent_ids,sentence_length)
            batches_of_sentence_ids.append(sent_ids)
...
def process_sentences(sequencelabeler, sentences, testing, learningrate, name, main_label_id, word2id, char2id, label2id, singletons, config, verbose=True,cutoff=0):
    """
    ...
    if cutoff != 0:
        ...
        percentage = 0
    batches_of_sentence_ids = create_batches_of_sentence_ids(sentences, config["max_batch_size"], cutoff=cutoff, percentage=percentage)
    if testing == False:
...
def run_experiment(config_path):
    ...
    if config["path_train"] is not None and len(config["path_train"]) > 0:
        try:
            assert config["path_train"].count(":") <= 1
        except:
            print("Input Error: More than 2 input files given!")
            print("Config path_train:",config["path_train"])
            raise
            
        sentences_train, split_points = read_input_files(config["path_train"], splits=True)
        if DEBUG:
            print("Split-points:",split_points)

        if len(split_points)>1:
            if split_points[0] >= 0.5*split_points[1]:
                sentences_train = sentence_train[split_points[0]+1:]+sentences_train[:split_points[0]+1]
                cutoff = split_points[1] - split_points[0] - 1 
    ...
    if config["path_train"] is not None and len(config["path_train"]) > 0:
        ...
        for epoch in xrange(config["epochs"]):
            ...
            random.shuffle(sentences_train[:split_points[0]+1])
            random.shuffle(sentences_train[split_points[0]+1:])
            for mini_batch_number,i in enumerate(range(split_points[0],split_points[1],split_points[0])):
                print("Epoch:"+str(epoch)+"\tmini-batch:"+str(mini_batch_number))
                sentences_train_mini_batch = sentences_train[:split_points[0]]+sentences_train[i:i+split_points[0]]

                results_train = process_sentences(sequencelabeler, sentences_train_mini_batch, testing=False, learningrate=learningrate, name="train", main_label_id=label2id[str(config["main_label"])], word2id=word2id, char2id=char2id, label2id=label2id, singletons=singletons, config=config, verbose=True, cutoff=split_points[0])
                ...
                if ((epoch == 0 and mini_batch_number == 0) or (main_measure_type == "high" and results_dev[main_measure] > best_selector_value) 
